...
EXCLUDE_EXTS = {".pyc", ".png", ".jpg", ".jpeg", ".gif", ".ico", ".svg", ".sqlite3"}
...
def rename_file_or_dir(path):
    ...
    # Check if 'vadaka' is in name, but not 'vaadaka'
    if re.search(r'(?<!a)(?<!A)vadaka', basename, re.IGNORECASE):
        new_basename = basename.replace('vadaka', 'vaadaka').replace('Vadaka', 'Vaadaka').replace('VADAKA', 'VAADAKA')
...
for root, dirs, files in os.walk(TARGET_DIR):
    ...
    for file in files:
        if any(file.endswith(ext) for ext in EXCLUDE_EXTS):
            continue
        if file in ('refactor_vaadaka.py', 'full_backup.sql', 'full_backup.json'):
            continue
